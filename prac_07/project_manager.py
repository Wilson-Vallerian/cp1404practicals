...
def main():
    """Manage projects in FILENAME and determine to rewrite it."""
    load_file()

    print(f"Welcome to Pythonic Project Management\nLoaded 5 projects from {FILENAME}")
...
def print_completed_projects():
    ...
    for completed_project in completed_projects:
        print(f"\t{Project(completed_project[0], completed_project[1], completed_project[2], completed_project[3], completed_project[4])}")


def print_incomplete_projects():
    """Print incomplete projects."""
    for incomplete_project in incomplete_projects:
        print(f"\t{Project(incomplete_project[0], incomplete_project[1], incomplete_project[2], incomplete_project[3], incomplete_project[4])}")
    ...
    for project in projects:
        if project[4] == 100:
            ...
            if project in completed_projects:
                completed_projects.remove(project)
...
def print_project_detail(filtered_projects, i):
    ...
    print(f"{Project(filtered_projects[i][0], filtered_projects[i][1], filtered_projects[i][2], filtered_projects[i][3], filtered_projects[i][4])}")
